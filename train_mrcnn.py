...
def train_mdnet():
    ...
    if pretrain_opts['set_type'] == 'IMAGENET':
        img_home = '/home/wilsontsang/tracking_benchmark/ILSVRC2015/Data/VID/train/'
        data_path = './modules/imagenet_refine_603.pkl'
    ...
    for i in range(pretrain_opts['n_cycles']):
        ...
        prec = np.zeros(K)
        totalTripleLoss = np.zeros(K)  # totalLoss
        ...
        for j, k in enumerate(k_list):
            ...
            # randomly selected K_perm
            K_perm = K_perm[0:pretrain_opts['n_brances_for_loss']] 
            # print(len(K_perm))
            for cidx in K_perm:
                if k == cidx:
                    continue
            ...
            (cls_loss+pretrain_opts['balance']*interclass_loss).backward() # 0.1
            # print(pretrain_opts['balance'])
            batch_cur_idx+=1
            if (batch_cur_idx%pretrain_opts['seqbatch_size'])==0:  # accumlate n iteration
        ...
        print "Mean Precision: %.3f Triple Loss: %.3f Inter Loss: %.3f IoU: %.3f" % (prec.mean(), 0, totalInterClassLoss.mean(),total_miou)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-set_type", default = 'IMAGENET' ) # VOT
    parser.add_argument("-padding_ratio", default = 5., type =float)
    parser.add_argument("-model_path", default ="./models/rt_mdnet_1221.pth", help = "model path")
    ...
    parser.add_argument("-n_cycles", default = 500, type = int ) #1000
